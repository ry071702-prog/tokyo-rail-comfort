...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fry071702-prog%2Ftokyo-rail-comfort)

> ↑ このボタンから環境変数なしでそのまま公開できます (運行情報はデモデータで動き、あとから ODPT / Supabase のキーを差せます  詳細は [docs/DEPLOY.md](docs/DEPLOY.md))
...
- **区間 × 時間帯の推定混雑ヒートマップ** — 3路線を切り替え、駅間ごとに 0〜23 時の推定混雑を5段階の色で表示  「今」の時間帯は枠で強調
- **選択区間の時間帯グラフ + 「空いている時間」トップ3** — この区間は何時なら空いているかを提案
- **リアルタイム運行情報バナー** — 遅延・運休をデータ生成時刻 (`dc:date`) と出典付きで表示  遅延分は「今」の推定混雑に補正として反映

ローカルで見る場合は下記セットアップの `npm run dev` → http://localhost:3000  (公開URL確定後に画面キャプチャを差し替える)
